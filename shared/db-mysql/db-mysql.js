...
function batchInsert(insertKey, objectArray, callback){
	...
	pool.getConnection(function(err, connection) {
		...
		for(var i=0; i<objectsToInsert; i++){
            ...
            var rowInsertString = prepare(insertString,object);
			if(debug) {
				console.log(rowInsertString, object);
			}
			// Use the connections
			connection.query(rowInsertString, object, function(err, result) {
				if (err) {
					console.log("erro inserting object :",object, rowInsertString);
					throw err;
				}
				insertedObjects++;
